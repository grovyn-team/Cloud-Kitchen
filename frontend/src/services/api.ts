...
// Resolution order: window.__RUNTIME_CONFIG__ (container env at startup, see
// docker-entrypoint.sh) → VITE_API_BASE_URL (build time) → '' (dev proxy).
// This lets one built image point at a different backend per client without a rebuild.
const runtimeApiBaseUrl =
  typeof window !== 'undefined' && typeof window.__RUNTIME_CONFIG__?.API_BASE_URL === 'string'
    ? window.__RUNTIME_CONFIG__.API_BASE_URL
    : '';
const buildApiBaseUrl =
  typeof import.meta.env.VITE_API_BASE_URL === 'string' ? import.meta.env.VITE_API_BASE_URL : '';
const baseURL = (runtimeApiBaseUrl || buildApiBaseUrl).replace(/\/+$/, '');
